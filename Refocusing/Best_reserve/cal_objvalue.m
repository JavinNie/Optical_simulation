...
%基本参数设置
ini_flag=1;%第一次遍历的标志
aim_intensity=zeros(px,1);%目标区域的光强
...
for count=1:px
    ...
    %%
    %如果已经计算过，避免重复计算
    if sum(ismember(pop(1:count-1,:),ind,'rows'))
        samevalue=aim_intensity(ismember(pop(1:count-1,:),ind,'rows'));
        aim_intensity(count,1)=samevalue(1);  
        continue
    end
    %%
    %如果没有计算过
